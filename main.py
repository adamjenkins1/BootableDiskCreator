...
class BootableDiskCreator:
    ...
    def progressCallback(self, bytesWritten):
        ...
        print('copying... {0}/{1} * 100 = {2}%'.format(self.totalBytesWritten, self.totalBytes, float(self.totalBytesWritten/self.totalBytes)*100))
        #print('copying... {0}%'.format(float(self.totalBytesWritten/self.totalBytes)*100))
    ...
    def getTotalBytes(self, location):
        self.totalBytes = int(self.executeCommand('getting size of mounted image...', 
                             'du -sb {0} | awk \'{{print $1}}\''.format(location)))
    ...
    def main(self):
        ...
        if os.path.ismount(isoMount):
            self.executeCommand('unmounting previously mounted iso...', 'umount {0}'.format(isoMount))
        ...
        self.getTotalBytes(isoMount)

        # format given partition and then mount it
        self.executeCommand('formatting partition as fat32...', 'mkfs.fat -F32 -I {0}'.format(device))
        self.executeCommand('mouting {0} to {1}...'.format(device, target),
        ...
        for entry in os.listdir(isoMount):
            ...
            if os.path.islink(isoEntry):
                ...
                shutil.copy(isoEntry, target)
        print('done')

        # final clean up
        self.executeCommand('unmounting image...', 'umount {0}'.format(isoMount))
